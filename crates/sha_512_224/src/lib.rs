...
//! SHA-512/224: the SHA-512 core with the SHA-512/224 IV and the output truncated
//! to the first 28 bytes. Structured identically to `sha_256`, but on 64-bit words:
//! 128-byte blocks, 80 rounds, an 80-entry message schedule, a 128-bit length
//! field, and a `[u64; 80]` scratch array reused across calls. The schedule and
//! compression loops are partially unrolled 8 at a time via macros, keeping the
//! same cache-friendly array-indexing access pattern.
...
/// A structure representing the SHA-512/224 hash algorithm.
...
impl Sha512_224 {
    /// Creates a new instance of the SHA-512/224 hash algorithm.
    pub fn new() -> Self {
        Self {
            w: [0; 80],
            h0: 0,
    ...
    /// Computes the SHA-512/224 digest of the given message.
    pub fn digest(&mut self, msg: &[u8]) -> [u8; 28] {
        self.h0 = 0x8c3d37c819544da2;
        self.h1 = 0x73e1996689dcd4d6;
        self.h2 = 0x1dfab7ae32ff9c82;
        self.h3 = 0x679dd514582f9fcf;
        self.h4 = 0x0f6d2b697bd44da8;
        self.h5 = 0x77e36f7304c48942;
        self.h6 = 0x3f9d85a86a1d36c8;
        self.h7 = 0x1112e6ad91d692a1;
        ...
        // SHA-512/224 emits the first 28 bytes: h0..h2 (24 bytes) + 4 bytes of h3.
        let mut hash = [0; 28];
        ...
        hash[24..28].copy_from_slice(&self.h3.to_be_bytes()[0..4]);
        hash
    }
}
...
mod tests {
    use super::*;
    use sha2::{Digest, Sha512_224 as Theirs};
    ...
    fn known_vectors() {
        ...
        assert_eq!(&empty[..4], &[0x6e, 0xd0, 0xdd, 0x02]); // SHA-512/224("")
